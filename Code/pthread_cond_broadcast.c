...
a program where 15 candies are to be bought. 1 thread produces 10 candies
each second and other threads wait for it to be enough candies to then
buy 15 candies eachs.
...
void *produce_candies()
{
    for (int i = 0; i < 5; i++)
    {
        ...
        pthread_cond_broadcast(&buying_cond); // signals when num_candies changes
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&threads[2], NULL, &buying_candies, NULL);
    pthread_create(&threads[3], NULL, &buying_candies, NULL);
